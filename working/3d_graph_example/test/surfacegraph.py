...
#from CSV_416 import MainWindow
...
class SurfaceGraph(QObject):
    ...
    def fillSqrtSinProxy(self):
        ...
        filename = ('C:/Users/RyanS/acconeer-python-exploration-master/working/4.21.h5')
        with h5py.File((filename), 'r') as f:
            ...
            stepX = record.session_info['step_length_m']
            stepZ = (max_sweep - min_sweep) / data_length
            ...
            x_list = depths.tolist()
            print(type(x_list[0]))
            x_float = x_list[0]
            
            self.m_sqrtSinProxy
            
            #y_float = float(y[0:data_length:1])
            #print(y_float)
            #y_float = time.tolist()

            sweeps.read_direct(z_float)
            dataArray = []
