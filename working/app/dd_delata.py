...
from acconeer.exptool.a111 import _clients, _configs, _utils
...
def main():
    args = _utils.ExampleArgumentParser(num_sens=1).parse_args()
    _utils.config_logging(args)

    if args.socket_addr:
        client = _clients.SocketClient(args.socket_addr)
    elif args.spi:
        client = _clients.SPIClient()
    else:
        port = args.serial_port or utils.autodetect_serial_port()
        client = _clients.UARTClient(port)
    ...
    interrupt_handler = _utils.ExampleInterruptHandler()
...
def get_sensor_config():
    config = _configs.EnvelopeServiceConfig()
...
class Processor:
    def __init__(self, sensor_config, processing_config, session_info):
        ...
        self.r = _utils.get_range_depths(sensor_config, session_info)
...
class PGUpdater:
    ...
    def setup(self, win):
        ...
        self.sweep_curve = self.sweep_plot.plot(
            pen=_utils.pg_pen_cycler(5),
            name="Envelope sweep",
        )

        self.mean_sweep_curve = self.sweep_plot.plot(
            pen=_utils.pg_pen_cycler(0, width=3),
            name="Mean Envelope sweep",
        )

        self.threshold_curve = self.sweep_plot.plot(
            pen=_utils.pg_pen_cycler(1),
            name="Threshold",
        )

        self.smooth_max_sweep = _utils.SmoothMax(
        ...
        for i in range(3):
            ...
            color_tuple = _utils.hex_to_rgb_tuple(utils.color_cycler(color_idx))
    ...
    def update(self, data):
        ...
        if len (peaks) > 1:
            ...
            if len (peaks) == 2:
                dif = float(sorted_peaks[1]) - float (sorted_peaks[0])
                text = f"{sorted_peaks[0]:2f} cm {sorted_peaks[1]:2f cm {dif}:.2f cm} {dif:.25} cm"
            elif len (peaks) == 3:
                dif = float(sorted_peaks[1]) - float(sorted_peaks[0])
                dif1 = float(sorted_peaks[2]) - float(sorted_peaks[0])
                text = f"{sorted_peaks[0]:.2f} cm {sorted_peaks[1]:.2f} cm {sorted_peaks[2]:.2f} cm\
                {dif:.2f} cm {dif1:.2f} cm"
            else:
                dif = float(sorted_peaks[1]) - float(sorted_peaks[0])
                dif1 = float(sorted_peaks[2]) - float(sorted_peaks[0])
                dif2 = float(sorted_peaks[3]) - float(sorted_peaks[0])
                text = f"{sorted_peaks[0]:.2f} cm {sorted_peaks[1]:.2f} cm {sorted_peaks[2]:.2f} cm\
                {sorted_peaks[3]:.2f} cm {dif:.2f)} cm {dif1:2f)} cm {dif2:.2f)} cm"
        elif len (peaks) == 1:
